...
export const getGenres = createAsyncThunk('MassMovies/genres', async () => {
    const {
        data: {genres},
    } = await axios.get(
...
moviesArray.push({
    ...
    export const fetchMovies = createAsyncThunk("MassMovies/trending", async ({type}, myThunk)=>{
        const {massmovies: {genres},} = myThunk.getState()
        return getMovieData(`${TMDB_BASE_URL}/trending/${type}/week?api_key=${MY_API_KEY}`, genres, true);
        // console.log(data)
    })

const MassMoviesSlice = createSlice({
    name: "MassMovies",
    initialState,
...
export const store = configureStore ({
    reducer: {
        massmovies: MassMoviesSlice.reducer
    }
})